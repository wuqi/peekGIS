...
- **每次改完代码就立刻提交, 不要问用户"要不要提交"**; 即使不确定对错也先提交(保险, 便于回退)。
- 一个主题一个提交; 多个不同主题的改动**分开提交** (可用 `git add <文件>` 分别提交, 或 `git add -p` 拆 hunk)。
- 只提交本次改动涉及的文件; **不要 `git add -A`** 把无关文件/生成物带进去。
- 提交信息用中文, 简述改了什么 (必要时加要点)。

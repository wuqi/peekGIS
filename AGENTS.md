...
- **禁止**把缓存、构建产物、测试输出、临时文件写到 `C:`(含 `%TEMP%` / `%LOCALAPPDATA%` / `C:\Users\...`)。
  ...
  - 运行期缓存(几何缓存 / v2 矢量瓦片 `.vtk` / 烘焙 `.pak`): 程序按 **exe 相对目录**定位, 即
    ...
    **凡是要给 peekgis 读的产物(如 `vt_build` 的 `.vtk`)必须放这里(用 `--auto`), 否则 app 发现不了。**
  - `build\tmp\` 仅用于**与缓存无关的临时文件**(诊断输出、一次性对比数据), 用完清理。
...
- **超大型矢量(v2 矢量瓦片缓存)工具**: `vt_build`(建缓存, `--auto` 写到 app 自动发现的 `cache/vtk/<hash8>_e<源>_d<显示>.vtk`)、`vt_estimate`(只读估算 N/各层点数/最细层/体积/路由)、`vt_tests`(v2 管线单测, 不依赖 lua/GL)。源文件打开时若存在匹配缓存会自动走瓦片渲染, 大文件无缓存时后台自动生成(`config.toml` `[vt] auto_build/threshold_verts`); 显示 CRS 不同则后台整块重投影。缓存管理窗口同时管理 v1.0 几何缓存与 v2 瓦片缓存。用法见 README「超大型矢量数据」与 `docs/矢量缓存方案.md`。